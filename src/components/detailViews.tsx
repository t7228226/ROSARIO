import { getTeamOfPerson, getOwnGroupDutyDisplay, qualificationBadge } from "../lib/selectors";
...
export function PersonDetailView({ person, qualifications, compact = false }: { person: Person; qualifications: Qualification[]; compact?: boolean }) {
  ...
  const wrapperClass = compact ? "compact-info-grid three-up" : "compact-info-grid three-up";
        ...
        {qualifications.map((item) => {
          ...
          return (
            <tr key={`${item.employeeId}-${item.stationId}`}>
              ...
              <td>{person && String(getTeamOfPerson(person)) === String(people.find((p) => p.id === item.employeeId)?.shift || "") ? "本班" : "支援"}</td>
              <td><span className={qualificationBadge(item.status)}>{item.status || "空白"}</span></td>
            </tr>
          );
        })}
