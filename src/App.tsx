...
export default function App() {
  ...
  useEffect(() => {
    const syncMobileState = () => setIsMobileView(window.innerWidth <= 900);
    syncMobileState();
        ...
        <div className="mobile-modal-backdrop" onClick={() => setMobileDetailModal(null)}>
          <div className="mobile-modal" onClick={(e) => e.stopPropagation()}>
            <div className="mobile-modal-header">
              <strong>
                {mobileDetailModal.type === "person" ? "人員資訊" : mobileDetailModal.type === "station" ? "站點資訊" : "站點考核"}
              </strong>
              <button type="button" className="mobile-modal-close" onClick={() => setMobileDetailModal(null)}>×</button>
            </div>
            <div className="mobile-modal-body">
              {mobileDetailModal.type === "person" && mobilePerson ? (
                <>
                  {(() => {
                    const duty = getPersonDutyDisplay(mobilePerson);
                    return <div className="detail-grid"><Info label="工號" value={mobilePerson.id} /><Info label="姓名" value={mobilePerson.name} /><Info label="職務" value={mobilePerson.role} /><Info label="班別" value={String(getTeamOfPerson(mobilePerson))} /><Info label="(A)第一天" value={duty.aDay1} /><Info label="(A)第二天" value={duty.aDay2} /><Info label="國籍" value={mobilePerson.nationality} /><Info label="(B)第一天" value={duty.bDay1} /><Info label="(B)第二天" value={duty.bDay2} /></div>;
                  })()}
                  <table className="table"><thead><tr><th>站點</th><th>狀態</th></tr></thead><tbody>{data.qualifications.filter((item) => item.employeeId === mobilePerson.id).map((item) => <tr key={`${item.employeeId}-${item.stationId}`}><td>{item.stationId}</td><td><span className={qualificationBadge(item.status)}>{item.status || "空白"}</span></td></tr>)}</tbody></table>
                </>
              ) : null}

              {mobileDetailModal.type === "station" && mobileStation ? (
                <>
                  <div className="detail-grid"><Info label="站點代碼" value={mobileStation.id} /><Info label="站點名稱" value={mobileStation.name} /><Info label="班別" value={stationTeamFilter} /><Info label="日別" value={stationDayFilter} /><Info label="本籍出勤" value={String(stationAttendance.localCount)} /><Info label="菲籍出勤" value={String(stationAttendance.filipinoCount)} /><Info label="越籍出勤" value={String(stationAttendance.vietnamCount)} /><Info label="總出勤" value={String(stationAttendance.totalCount)} /><Info label="當班人力" value={String(stationAttendance.own.length)} /><Info label="支援人力" value={String(stationAttendance.support.length)} /><Info label="支援對班" value={stationDayFilter === "當班" ? "-" : stationAttendance.supportTeam} /><Info label="支援代號" value={stationDayFilter === "當班" ? "-" : stationAttendance.supportDuty} /></div>
                  <table className="table"><thead><tr><th>工號</th><th>姓名</th><th>班別</th><th>來源</th><th>資格</th></tr></thead><tbody>{stationScopedQualifications.filter((item) => item.stationId === mobileStation.id).map((item) => { const person = data.people.find((p) => p.id === item.employeeId); return <tr key={`${item.employeeId}-${item.stationId}`}><td>{item.employeeId}</td><td>{person?.name || item.employeeName || "-"}</td><td>{person ? String(getTeamOfPerson(person)) : "-"}</td><td>{stationAttendance.own.some((p) => p.id === item.employeeId) ? "當班" : "支援"}</td><td><span className={qualificationBadge(item.status)}>{item.status || "空白"}</span></td></tr>; })}</tbody></table>
                </>
              ) : null}
              ...
              {mobileDetailModal.type === "review" && mobileReviewPerson ? (
                <>
                  {(() => {
                    const duty = getPersonDutyDisplay(mobileReviewPerson);
                    return <><div className="detail-grid"><Info label="工號" value={mobileReviewPerson.id} /><Info label="姓名" value={mobileReviewPerson.name} /><Info label="班別" value={String(getTeamOfPerson(mobileReviewPerson))} /><Info label="職務" value={mobileReviewPerson.role} /><Info label="系統權限" value={String(getSystemPermission(mobileReviewPerson) || "-")} /></div><div className="detail-grid"><Info label="(A)第一天" value={duty.aDay1} /><Info label="(A)第二天" value={duty.aDay2} /><Info label="(B)第一天" value={duty.bDay1} /><Info label="(B)第二天" value={duty.bDay2} /></div></>;
                  })()}
                  <div className="form-grid compact-form"><div><label className="field-label">站點</label><select value={reviewStationId} onChange={(e) => setReviewStationId(e.target.value)}><option value="">請選擇站點</option>{data.stations.map((station) => <option key={station.id} value={station.id}>{station.id}｜{station.name}</option>)}</select></div><div><label className="field-label">狀態</label><select value={reviewStatus} onChange={(e) => setReviewStatus(e.target.value as QualificationStatus)}>{qualificationOptions.map((item) => <option key={item || "blank"} value={item}>{item || "空白"}</option>)}</select></div></div>
                  <div className="toolbar"><button className="primary" type="button" onClick={() => handleSaveQualification()}>確認並儲存</button></div>
                  <table className="table"><thead><tr><th>站點</th><th>狀態</th><th>操作</th></tr></thead><tbody>{data.qualifications.filter((item) => item.employeeId === mobileReviewPerson.id).map((item) => <tr key={`${item.employeeId}-${item.stationId}`}><td>{item.stationId}</td><td><span className={qualificationBadge(item.status)}>{item.status || "空白"}</span></td><td><button className="danger" type="button" onClick={() => handleDeleteQualification(item.employeeId, item.stationId)}>刪除</button></td></tr>)}</tbody></table>
                </>
              ) : null}
            </div>
          </div>
